 ...
 *
 *     .cards
 *       span.cards__aura — свет по контуру, лежит слоем 0
  ...
  return (
    <section id="pricing" className="section section--clipx">
      <div className="shell">
        ...
        >
          {/* СВЕТ ПО КОНТУРУ ЛЕЖИТ ОТДЕЛЬНЫМИ СЛОЯМИ И ИДЁТ ПЕРВЫМ.
              Внутри карты он оказался бы в её контексте наложения,
              и ореол второй лёг бы поверх первой: слоты рисуются
              целиком по очереди (Р-68). Ячейку задаёт `grid-area`,
              наклон — тот же цикл, что ведёт саму карту. */}
          {PLANS.map((p, i) => (
            <span key={`aura-${p.id}`} className="cards__aura" data-i={i} aria-hidden="true" />
          ))}
