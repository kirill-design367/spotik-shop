 ...
 */
const DOKUMENTY: [string, string, string][] = [
  /* ⚠️ НАЗВАНИЯ УКОРОЧЕНЫ РОВНО НАСТОЛЬКО, ЧТОБЫ СТРОКА ВСТАЛА В ДВЕ
     НА 390, И УКОРОЧЕНЫ ЗАМЕРОМ, А НЕ НА ГЛАЗ: два названия по 27
     знаков не встают на одну строку ни с чем при поле в 350 px,
     и четыре полных имени давали три строки. Полное название каждого
     документа стоит его заголовком и уходит в `aria-label`: здесь
     нужна опись, а не цитата. */
  ['/oferta/', 'Оферта', 'Публичная оферта'],
  ['/soglashenie/', 'Пользовательское соглашение', 'Пользовательское соглашение'],
  ['/politika/', 'Конфиденциальность', 'Политика конфиденциальности'],
  ['/soglasie/', 'Согласие на обработку', 'Согласие на обработку персональных данных'],
];
...
export default function Footer() {
  ...
  return (
    ...
    >
      ...
      {/* ⚠️ ССЫЛКИ НА ДОКУМЕНТЫ — ОБЯЗАТЕЛЬНЫЙ РЕКВИЗИТ, а не вежливость:
          оферта, соглашение, политика и согласие должны быть доступны
          оттуда, где стоят реквизиты, и прятать их за нажатие нельзя.

          ⚠️ И ЛЕЖАТ ОНИ СТРОКОЙ, А НЕ СТОЛБИКОМ (тридцать седьмая
          итерация): столбик из четырёх длинных названий читался как
          пятый блок реквизитов. Разделитель — волосяная черта, и она
          нарисована НА ЭЛЕМЕНТЕ СПИСКА, а не внутри ссылки: внутри
          она попала бы в кликабельную область. */}
      <nav className="footer__docs shell" aria-label="Документы">
        {DOKUMENTY.map(([href, imya, polnoe]) => (
          <span key={href} className="footer__doc">
            <a href={href} aria-label={polnoe}>
              {imya}
            </a>
          </span>
        ))}
