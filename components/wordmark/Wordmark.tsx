...
export default function Wordmark({ mode, sectionId, followSelector }: Props) {
  ...
  useEffect(() => {
    ...
    const origin = (p: number): number => {
      if (!baseSet) {
        /* Начало отсчёта берётся с первого НЕНУЛЕВОГО прогресса, а не
           с первого вызова: подписка отдаёт текущее значение сразу при
           заводе, и на этом шаге оно может быть ещё нулевым — смещение
           тогда окажется нулевым ровно там, где оно и нужно. */
        if (p <= 0) return 0;
        baseSet = true;
