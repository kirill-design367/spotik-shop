 ...
 * она меньше.
 *
 * ⚠️ ЧИСЛО ПОДОБРАНО ЗАМЕРОМ, И ОДИН ЗАХОД ПО ДОРОГЕ ОКАЗАЛСЯ
 * ЛОЖНОЙ ТРЕВОГОЙ — это стоит записать. На 2560 двадцать четыре
 * куска снимают цену ленты на скролле до нуля: 1.4 % кадров дороже
 * 16.9 мс против 1.5 % без неё, длинных задач нет вовсе (было 33 %
 * и полтора десятка задач). Мобильный проход тачем при этом
 * показывал вдвое худший результат, и мы успели проверить три
 * гипотезы — перестановку классов, число кусков и нарезку
 * по ломаной вместо кубиков. Ни одна не подтвердилась, а решил
 * дело КОНТРОЛЬНЫЙ ОПЫТ: один кусок, то есть исходное поведение,
 * в тех же условиях дал ровно те же числа. Просела сама среда
 * за сессию. Отсюда правило: сравнивать мобильные прогоны можно
 * только между собой и только в один присест.
 *
 * Резать мельче на мобильной нечего, а число кусков глазом
 * не видно ни там, ни там: рез приходится в пропуск пунктира.
 */
const CHUNKS = 24;
/**
...
export default function Route({ steps }: { steps: Step[] }) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      built = buildPath(h, bleed, over, vw, ax, ys, boxes, CHUNKS);
