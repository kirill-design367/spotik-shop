...
const nextConfig = {
  ...
  trailingSlash: true,
  // ⚠️ СТАРЫЙ АДРЕС ОСТАЁТСЯ ЖИВЫМ И ВЕДЁТ ТЕПЕРЬ ПО НАЗНАЧЕНИЮ.
  // `/privacy/` был заглушкой политики конфиденциальности; пока
  // настоящего текста не было, он вёл на оферту — за неимением
  // лучшего. С тридцать четвёртой итерации политика на сайте есть,
  // и адрес ведёт на неё: ссылки на прежний адрес уже могли уехать
  // наружу, и приводить человека не туда, куда он шёл, незачем.
  async redirects() {
    return [{ source: '/privacy', destination: '/politika/', permanent: true }];
  },
