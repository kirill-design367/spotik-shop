 ...
 * менять нечего.
 *
 * ⚠️ НА БОЕВОМ СЕРВЕРЕ `SMTP_HOST` УЖЕ ЗАДАН, ТО ЕСТЬ ЭТА ВЕТКА ТАМ
 * ВЫКЛЮЧЕНА. Значит запасного пути «код в журнале» на бою больше нет:
 * не ушло письмо — войти нельзя. Сейчас так и есть — Яндекс отвечает
 * `535 Invalid user or password` на шаге `AUTH PLAIN` (Р-99). Печатать
 * код в журнал и при НЕУДАЧНОЙ отправке — отдельная постановка,
 * а не мелкая правка: временный путь такого рода живёт годами (Р-91),
 * и на бою он означает вход в любой кабинет для всякого, у кого есть
 * доступ к серверу.
 *
