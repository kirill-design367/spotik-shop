...
export function attachCards(root: HTMLElement, canvas: HTMLCanvasElement): () => void {
  ...
  const tick = (now: number) => {
    ...
    /* Кадр в покое реже: дыхание медленное, и 24 в секунду от 60
       на нём не отличить, а площадь заливки — главный расход. */
    if (!moving && !dirty && now < nextAt) {
      pump();
      return;
    }
    last = now;
    nextAt = now + (moving ? 0 : IDLE_MS);
    dirty = false;
