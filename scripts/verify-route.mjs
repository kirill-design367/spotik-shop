...
/* Профиль можно взять один: `node scripts/verify-route.mjs 390`.
   Прогон целиком идёт около пяти минут, и при разборе одного размера
   гонять три незачем. */
const ODIN = process.argv[2];
for (const [w, h, mob] of [
  [390, 844, true],
  [1920, 1080, false],
  [2560, 1440, false],
].filter((p2) => !ODIN || String(p2[0]) === ODIN)) {
        ...
        if (!on && y0 >= 0) {
          ...
          if (y0 > 0 && y1 < A.height - 1 && L >= 8 && Math.abs(X[y1] - X[y0]) <= L * 0.4) {
            ...
            for (let k = y0; k <= y1; k += 1) {
              pk = Math.max(pk, P[k]);
              /* ⚠️ СУММА БЕРЁТСЯ ПО СВЯЗНОМУ ПЯТНУ ВОКРУГ ЯДРА,
                 А НЕ ПО ОКНУ ФИКСИРОВАННОЙ ШИРИНЫ. Лента петляет,
                 и на повороте соседняя прядь входит в окно ±24 px:
                 сумма удваивается, а штрих при этом целый. Ровно
                 на этом сторож и упал на раннере — 1.46 медианы
                 при 1.36 на той же сборке здесь. Микроточка, ради
                 которой сумма и считается, лежит ВНУТРИ штриха
                 (два скруглённых конца в одном месте) и связна
                 с ядром по построению, поэтому обход в обе стороны
                 до первого тёмного пикселя её не теряет. */
              let sm = gv(X[k], k);
              for (let x = X[k] - 1; x >= Math.max(0, X[k] - 24); x -= 1) {
                const v = gv(x, k);
                if (v <= 1) break;
                sm += v;
              }
              for (let x = X[k] + 1; x < Math.min(A.width, X[k] + 25); x += 1) {
                const v = gv(x, k);
                if (v <= 1) break;
                sm += v;
              }
  ...
  /* Пороги взяты из КОНТРОЛЬНОГО ПРОГОНА НА ПРЕЖНЕМ КОДЕ: там среди
     ...
     ⚠️ СУДИТСЯ ФОРМА ДЕФЕКТА, А НЕ ОДИН ОТСЧЁТ, и это правка
     тридцать четвёртой. Рез приходится на КАЖДУЮ границу куска,
     поэтому настоящий дефект задевает разом почти половину
     промеренных штрихов — в контроле четыре из восьми. Одиночное
     превышение так не выглядит: оно приходит из растра и из среды,
     и ровно на нём сторож упал на раннере (1.46 при 1.36 на той же
     сборке здесь, выкладка № 116). Поэтому одно место терпится,
     два — уже нет, и отдельно стоит потолок: выше 1.55 не терпится
     ни одно, это верх контрольного прогона. */
  const okShtrih =
    lenta.runs >= 5 &&
