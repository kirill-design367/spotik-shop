...
for (const [w, h, mob] of SIZES) {
  const shots = async (hide) => {
    /* ⚠️ СТРАНИЦА СНИМАЕТСЯ НЕПОДВИЖНОЙ, И ЭТО НЕ ПОБЛАЖКА.
       Сторож сравнивает ДВА РАЗНЫХ ОТКРЫТИЯ страницы — с шапкой
       и без неё, — и всё, что движется само, в них оказывается
       в разной фазе: бегущая строка, огни ленты, кольца микроволн
       у цифр. Пиксель под шапкой классифицируется по одному кадру,
       а меряется по другому — и сторож падает на собственном
       артефакте. Двадцать шестая итерация поймала это ровно
       на кольцах: четыре положения из 225 на 2560, все в блоке
       порядка. «Уменьшить движение» — это НАСТОЯЩЕЕ состояние
       страницы, а не подмена: содержимое то же самое, лента
       подсвечена целиком, просто ничто не едет. */
