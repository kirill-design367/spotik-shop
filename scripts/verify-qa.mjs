 ...
 * координатами всех вопросов до и после включения строки.
...
for (const [w, h, mob] of [
  ...
  /* Число вопросов НЕ ЗАШИТО: их четыре с тридцать четвёртой
     итерации и станет другим на следующей постановке. Сторож
     требует, чтобы они вообще были и чтобы КАЖДЫЙ ответ шёл одной
     строкой без переносов. */
  const okOne = lines.length >= 3 && lines.every((l) => l.wrap === 'nowrap' && l.rows === 1);
  ...
  const across = await (async () => {
    ...
    const on = await page.evaluate(() => {
      ...
      return {
        ...
        /* ⚠️ СРАВНИВАТЬ НАДО С МЕНЬШЕЙ ИЗ ДВУХ ВЫСОТ. Полоса выросла
           вместе с кеглем строки и на узком экране стала ВЫШЕ
           однострочного вопроса: требовать, чтобы она уложилась
           в его набор целиком, — значит требовать, чтобы строка
           была мельче вопроса. Перекрытие «поперёк» — это когда
           меньшая из двух высот накрыта полностью. */
        tickH: Math.min(tb.height, qb.height),
        color: cs.color,
  ...
  const okAcross =
    ...
    /* ⚠️ КЕГЛЬ И ПРИГЛУШЕНИЕ ПЕРЕСМОТРЕНЫ В ДВАДЦАТЬ ШЕСТОЙ ИТЕРАЦИИ:
       «строка крупнее и светлее, вопрос за ней почти исчезает».
       Было 19…28 px при приглушении до 0.36 — стало 23…35 при 0.12.
       Нижняя граница приглушения оставлена ненулевой намеренно:
       вопрос обязан оставаться НА МЕСТЕ и угадываться, иначе это
       снова подмена, а её сняли законом 26. */
    across.size >= 22 &&
    across.bright > 0.98 &&
    across.dim <= 0.2 &&
    across.dim >= 0.06;
