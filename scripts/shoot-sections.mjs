...
for (const [name, w, h] of [['mobile', 390, 844], ['desktop', 1920, 1080]]) {
  ...
  const diag = await page.evaluate(() => ({
    ...
    gl: document.querySelectorAll('.cards__glow').length,
  }));
  console.log(`${name} ${w}x${h}: документ ${diag.docW} / окно ${diag.winW} ${diag.docW > diag.winW + 1 ? '!!! ГОРИЗОНТАЛЬНЫЙ СКРОЛЛ' : 'скролла по X нет'}, высота ${diag.docH}, слоёв света за картами: ${diag.gl}`);
