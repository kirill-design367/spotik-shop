 ...
 * и есть его цена.
 *
 *   1. ПОДСВЕТКА КАРТОЧЕК В ПОКОЕ. Страница стоит, четыре карточки
 *      на экране. Слой подсветки статичен, и замер обязан это показать.
 *   2. ПОДСВЕТКА КАРТОЧЕК ПРИ ДВИЖЕНИИ. Указатель идёт по всем четырём:
 *      карточка наклоняется, её ореол попадает в ту же область
 *      перерисовки. Наклон включён в обеих строках.
 *   3. НАКЛОН И ГЛЯНЕЦ ЧЕТЫРЁХ КАРТОЧЕК.
...
for (const [name, w, h, mob, cpu] of PROFILES.filter((p) => !ONLY || String(p[1]) === ONLY)) {
  ...
  /* ⚠️ ПОДСВЕТКА ВЫКЛЮЧАЕТСЯ ЦЕЛИКОМ, А НАКЛОН — ТОЛЬКО ОТРИСОВКОЙ.
     Слой подсветки ничего не считает, у него меняется одна
     прозрачность, поэтому его можно просто убрать. У наклона наоборот:
     модуль продолжает считать и писать переменные в обеих строках,
     снимается только трансформ и пятно — значит разница между строками
     это цена ОТРИСОВКИ приёма, а не его арифметики. Раскладка
     при этом не меняется ни на пиксель. */
  const NOGLOW = '.slot__glow{display:none!important}';
  const NOTILT = '.card{transform:none!important}.card__sheen{display:none!important}';
  ...
  await scene(page, '1. ПОДСВЕТКА КАРТОЧЕК В ПОКОЕ (страница стоит)', NOGLOW, () =>
  ...
  if (!mob) {
    ...
    /* Подсветка при ДВИЖЕНИИ: карточка наклоняется, её ореол лежит
       под ней и попадает в ту же область перерисовки. Наклон в обеих
       строках включён — меряется добавка от подсветки. */
    await scene(page, '2. ПОДСВЕТКА КАРТОЧЕК ПРИ ДВИЖЕНИИ (указатель идёт по всем)', NOGLOW, sweep);
    await scene(page, '3. НАКЛОН И ГЛЯНЕЦ ЧЕТЫРЁХ КАРТОЧЕК', NOTILT, sweep);
  }
