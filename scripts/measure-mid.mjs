/**
 * ЦЕНА ТРЁХ НОВЫХ ПРИЁМОВ СЕРЕДИНЫ — ОТДЕЛЬНЫМ ЗАМЕРОМ.
 *
 ...
 * и есть его цена.
 *
 *   1. БЕГУЩАЯ СТРОКА В ПОКОЕ. Страница стоит, строка идёт. Это
 *      единственное место на сайте, где в покое что-то движется,
 *      поэтому мерить его надо отдельно от прокрутки.
 *   2. БЕГУЩАЯ СТРОКА НА ПРОКРУТКЕ. Фон под слоем выворотки меняется
 *      каждый кадр, и `backdrop-filter` обязан перечитывать подложку.
 *   3. РАЗВОРОТ РЯДА. 620 мс перехода на четырёх трансформах.
 *   4. ПАНЕЛЬ БЛОКА 3. Слой выворотки едет вместе с панелью.
 *
...
for (const [name, w, h, mob, cpu] of [['ДЕСКТОП 1920', 1920, 1080, false, 1], ['ДЕСКТОП 2560', 2560, 1440, false, 1], ['МОБИЛЬНАЯ ЭМУЛЯЦИЯ', 390, 844, true, 4]]) {
  ...
  await park('.plans', 0.15);
  await page.mouse.move(Math.round(w / 2), Math.round(h * 0.92));
  await scene(page, '1. БЕГУЩАЯ СТРОКА В ПОКОЕ (страница стоит)',
    '.mq{display:none!important}', () => page.waitForTimeout(3000));

  await scene(page, '2. БЕГУЩАЯ СТРОКА НА ПРОКРУТКЕ',
  ...
  if (!mob) {
    await park('.plans', 0.05);
    await scene(page, '3. РАЗВОРОТ РЯДА (переход 620 мс, четыре трансформа)',
      '.plan{transition:none!important}', async () => {
        /* Наводим на ШАПКУ ряда, а не на сам ряд: ряды наложены друг
           на друга, и центр бокса нижнего ряда закрыт следующим —
           это не дефект, а устройство приёма. */
        for (let i = 0; i < 4; i += 1) {
          await page.hover(`.plan__head >> nth=${i}`);
          await page.waitForTimeout(700);
        }
      });
  }

  await scene(page, '4. ПАНЕЛЬ БЛОКА 3 (слой выворотки едет с панелью)',
    '.steps__ink{display:none!important}', async () => {
