...
for (const [name, w, h, mob, cpu] of PROFILES.filter((p) => !ONLY || String(p[1]) === ONLY)) {
  ...
  /* ⚠️ СЦЕНА ВЫКЛЮЧАЕТСЯ ЦЕЛИКОМ, А НАКЛОН ТЕКСТА — ТОЛЬКО ТРАНСФОРМОМ.
     Объём, дыхание, глянец и ореол живут в ОДНОМ холсте, и врозь их
     из CSS не выключить: разница между строками — цена всей сцены.
     Именно её и просили замерить отдельно.

     У наклона наоборот: модуль в обеих строках считает и пишет
     переменные, снимается только сам трансформ карточки — значит
     разница это цена ПЕРЕРИСОВКИ текста, а не арифметики. Раскладка
     при этом не меняется ни на пиксель. */
  const NOGL = '.cards__gl{display:none!important}';
  const NOTILT = '.card{transform:none!important}';
  ...
  await scene(page, '1. СЦЕНА КАРТОЧЕК В ПОКОЕ: ЧЕТЫРЕ ПЛИТЫ ДЫШАТ (страница стоит)', NOGL, () =>
  ...
  if (!mob) {
    ...
    await scene(page, '2. СЦЕНА КАРТОЧЕК ПОД УКАЗАТЕЛЕМ (наклон, глянец, ореол)', NOGL, sweep);
    await scene(page, '3. НАКЛОН ТЕКСТА КАРТОЧКИ (перерисовка HTML поверх сцены)', NOTILT, sweep);
  }
