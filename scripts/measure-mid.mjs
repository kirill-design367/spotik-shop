...
for (const [name, w, h, mob, cpu] of PROFILES.filter((p) => !ONLY || String(p[1]) === ONLY)) {
  ...
  /* ⚠️ ПЕРЕЛИВ ПО КАЙМЕ ВЫКЛЮЧАЕТСЯ СНЯТИЕМ АНИМАЦИИ, А НЕ СНЯТИЕМ
     СЛОЯ. Клин остаётся нарисованным и стоит на нуле поворота:
     картинка та же, раскладка та же, разница между строками — цена
     ровно ДВИЖЕНИЯ. Это единственное, что на странице крутится
     само, поэтому меряется оно и в покое, и на прокрутке. */
  const NOSHEEN = '.card__edge::before{animation:none!important}';
  await scene(page, '1б. ПЕРЕЛИВ ПО КАЙМЕ ВЫБРАННОЙ В ПОКОЕ (страница стоит)', NOSHEEN, () =>
    page.waitForTimeout(4000),
  );

  await scene(page, '1в. ПЕРЕЛИВ ПО КАЙМЕ НА ПРОКРУТКЕ', NOSHEEN, async () => {
    await park('.cards', 0.9);
    await wheel(h * 2.2, 1500);
