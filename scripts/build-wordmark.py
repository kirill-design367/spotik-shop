...
# а не шрифтовой и не проектной. Слово тянется по вертикали независимо
# от горизонтали, поэтому шрифтовая прописная (750), проектная
# (CAP_RATIO × 1000) и ВИДИМАЯ — три разные величины. Берётся видимая,
# см. VIEW_CAP ниже.
GAP_DEPTH = float(os.environ.get('WM_GAP_DEPTH', '0.27'))
# Высота прописной НА ЭКРАНЕ, в долях ширины слова.
#
# ⚠️ ЭТО НЕ CAP_RATIO, И ПУТАТЬ ИХ НЕЛЬЗЯ — на этой путанице
# и держалась жалоба «на телефоне просветы разные». CAP_RATIO = 0.235
# описывает ПРОЕКТНОЕ пространство: в нём слово шириной 1000 единиц
# и прописная 235. А на экране слово тянется по вертикали независимо
# от горизонтали (`preserveAspectRatio="none"`, ширина в cqw, высота
# из `--wm-h`), и видимая прописная относительно ширины слова
# получается совсем другой: на 390×844 это 535 px при ширине 385, то
# есть 1.39, а на 1920 — 728 при 1895, то есть 0.38.
#
# Глубина обрезки задана В ДОЛЯХ ВИДИМОЙ прописной — так и написано
# в комментарии к GAP_DEPTH с пятой итерации, — но считалась она
# от проектной. Значит замер обрезался на 6.3 % ширины слова вместо
# 37 %, то есть карманы под перекладиной T и в развилке K почти
# не учитывались ни на одном настоящем экране.
#
# ⚠️ ВЗЯТА ПРОПОРЦИЯ ТЕЛЕФОНА, А НЕ ДЕСКТОПА, И ЭТО ЗАКОН 13:
# мобильная композиция первична. Разница между ними при этом
# невелика — мера насыщается примерно с 0.8 ширины слова, и обе
# пропорции дают почти одно и то же.
VIEW_CAP = float(os.environ.get('WM_VIEW_CAP', '1.39'))
...
def optical_offsets(per_letter, natural, n=320):
    ...
    depth = GAP_DEPTH * VIEW_CAP * NORM_WIDTH / sx_est
...
def main():
    ...
    print('Глубина обрезки %.2f ВИДИМОЙ высоты прописной (%.2f ширины слова).'
          % (GAP_DEPTH, GAP_DEPTH * VIEW_CAP))
