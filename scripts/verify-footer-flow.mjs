 ...
 *      В ПОЛНОСТЬЮ СЖАТОМ состоянии и не растёт;
 *   2) ВЕРХ чернил доходит до своей линии и ПРИБИВАЕТСЯ;
 *   3) с этого же кадра, без паузы, скролл растягивает слово ВНИЗ;
 *   4) слово раскрылось — растяжение кончилось;
 *   5) только после этого снизу выезжает мелкий текст.
 *
...
for (const dev of SIZES) {
  ...
  /* Верх чернил меряется там, где сцена ПРИЛИПЛА: до этого вся секция едет
  ...
  const bs = stuck.map((r) => r.top);
  const spread = bs.length ? Math.max(...bs) - Math.min(...bs) : NaN;
  const line = bs.length ? bs[0] : NaN;

  /* Зазор между прибиванием верха и началом роста — главное число пункта 2.
     Кадры, где верх уже стоит на своей линии, а форма ещё не двинулась. */
  let gap = 0;
  for (let i = 1; i < H.length; i += 1) {
    if (i >= firstGrow) break;
    if (Math.abs(H[i].top - line) < 0.75 && H[i].y !== H[i - 1].y) gap += 1;
  }
  ...
  /* Шаг 1 задания: на подходе слово обязано быть ПОЛНОСТЬЮ СЖАТЫМ.
     Раньше оно въезжало раскрытым, пока гидратация не поправит. */
  const approachMax = firstGrow > 1
    ? Math.max(...H.slice(0, firstGrow).map((r) => r.h)) : hTight;
  if (approachMax > hTight + 0.01) {
    failed += 1;
    console.log(`  ПРОВАЛ: ${dev.w}: на подходе слово не сжато (${approachMax.toFixed(1)} против ${hTight.toFixed(1)})`);
  }

  out.push({ dev, rows: H.length, hOpen, hTight, line, dead, moved, spread,
    stuck: stuck.length, gap, visibleIdle, approachMax });
  ...
  // в verify-footer.mjs.
  if (spread > 0.02) { failed += 1; console.log(`  ПРОВАЛ: ${dev.w}: верх чернил гуляет на ${spread.toFixed(2)} px на прилипшей сцене`); }
  if (dead['рост'] > 0) { failed += 1; console.log(`  ПРОВАЛ: ${dev.w}: ${dead['рост']} мёртвых кадров ВНУТРИ роста`); }
  if (gap > 0) { failed += 1; console.log(`  ПРОВАЛ: ${dev.w}: ${gap} кадров между прибиванием верха и началом роста`); }
}

console.log('── ВЕРХ ЧЕРНИЛ НА ПРИЛИПШЕЙ СЦЕНЕ ──────────────────────────────');
console.log('размер       линия верха   разброс   кадров');
for (const r of out)
...
console.log('кадров, где верх уже на своей линии, а форма ещё стоит:');
for (const r of out)
  console.log('  %s %s', String(r.dev.w + '×' + r.dev.h).padEnd(11), r.gap);
console.log('');
console.log('── НА ПОДХОДЕ СЛОВО СЖАТО ──────────────────────────────────────');
console.log('(высота формы в единицах контура: она читается прямо из атрибута d)');
for (const r of out)
  console.log('  %s высота на подходе %s ед., сжатое состояние %s ед.',
    String(r.dev.w + '×' + r.dev.h).padEnd(11),
    r.approachMax.toFixed(1).padStart(7), r.hTight.toFixed(1));
console.log('');
